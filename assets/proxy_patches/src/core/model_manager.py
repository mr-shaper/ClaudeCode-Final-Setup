...
class ModelManager:
    ...
    def map_claude_model_to_openai(self, claude_model: str) -> str:
        ...
        # Passthrough for Thinking models and specific providers
        if "thinking" in claude_model:
            return claude_model
            
        # If it's other supported models (ARK/Doubao/DeepSeek/Kimi/Gemini), return as-is
        if (claude_model.startswith("ep-") or claude_model.startswith("doubao-") or 
            claude_model.startswith("deepseek-") or claude_model.startswith("kimi-") or
            claude_model.startswith("gemini-")):
            return claude_model
