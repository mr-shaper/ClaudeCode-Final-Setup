...
from src.conversion.doc_parser import parse_document_content
import logging
...
def convert_claude_user_message(msg: ClaudeMessage) -> Dict[str, Any]:
    ...
    for block in msg.content:
        ...
        elif block.type == Constants.CONTENT_DOCUMENT:
             # Use doc_parser to extract text
             extracted_text = parse_document_content(
                 block.source.get("media_type", "unknown"),
                 block.source.get("data", "")
             )
             openai_content.append(
                {
                    "type": "text",
